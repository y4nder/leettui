...
- `HelpPopup.tsx` — Focus-aware keybinding list (Stage 11). Two sections built from the keymap's static scope seam (`describeScope`/`formatKeys`/`isScopeEntryVisible`, not runtime `getCommandEntries()`): **Local Keys** for the currently focused browse panel (changes with `focusedPanel`) and **Global Keys** (`browseGlobalBindings`). Debug entries only appear when `LEETTUI_DEBUG=1`. Opened with `?`.
