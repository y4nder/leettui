...
- `paths.ts` — XDG-compliant path constants (`CONFIG_DIR`, `DATA_DIR`, `CONFIG_FILE`, `DB_PATH`, `SOLUTIONS_DIR`, `TEMPLATES_DIR` — the latter `~/.config/leettui/templates/`, root of the per-language template-override tree)
