...
```bat
curl -fsSL https://raw.githubusercontent.com/y4nder/leettui/main/install.cmd -o install.cmd && install.cmd && del install.cmd
```

Same installer — `install.cmd` just runs the PowerShell script above under the hood, so it installs to the same place and honors the same overrides (set them first with `set LEETTUI_INSTALL_DIR=...` / `set LEETTUI_VERSION=v0.1.0`). `curl` ships with Windows 10 and later.
